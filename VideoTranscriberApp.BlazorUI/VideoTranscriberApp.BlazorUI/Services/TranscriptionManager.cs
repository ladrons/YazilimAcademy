...
using Xabe.FFmpeg;
using YA.VideoTranscriberApp.BlazorUI.Controllers;
using VideoTranscriberApp.BlazorUI.Client.Models;

namespace VideoTranscriberApp.BlazorUI.Services
{
    public class TranscriptionManager
    {
        ...
        public async Task<List<TranscriptionModel>> UploadAsync(VideoUploadRequest request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var englishTranscription = await TranscribeAudioAsync(audioFilePath, cancellationToken);

                var transcriptions = await TranslateTranscriptionsAsync(englishTranscription, request.Languages, cancellationToken);
        ...
        private async Task<List<TranscriptionModel>> TranslateTranscriptionsAsync(string transcription, string[] languages, CancellationToken cancellationToken)
        {
        ...
        private async Task<TranscriptionModel> TranslateTranscriptionAsync(string transcription, string language, CancellationToken cancellationToken)
        {
        ...
        private async Task<string> TranscribeAudioAsync(string audioFilePath, CancellationToken cancellationToken)
        {
